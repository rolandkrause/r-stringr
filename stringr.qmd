...
```{r read data}
agro_data <- read_csv("PubChem_Agrochemicals_20231022.csv")
```
...
(encoded by the `#` in the SMILES - canonical or isomeric).
Use `str_view` to look at those that are also salts (encoded by `.`).

```{r salts and triple bonds}
# Write your solution here
```


How many agrochemicals contain fluorine? Hint: look at the `mf` column
...
Luxembourg contributors. Use the `sidsrcname` column to detect Luxembourg. 
...
Finally, how many agrochemicals were contributed in 2023? Use the `cidcdate`
